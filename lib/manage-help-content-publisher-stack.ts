...
export class ManageHelpContentPublisherStack extends GuStack {
  constructor(scope: App, id: string, props: ManageHelpContentPublisherStackProps) {
    ...
    const deploymentBucket = Bucket.fromBucketName(this, 'DeploymentBucket', 'membership-dist');
    ...
    const publisherLambda = new LambdaFunction(this, 'PublisherLambda', {
      ...
      code: Code.fromBucket(deploymentBucket, `membership/${stage}/${app}/${app}.jar`),
      memorySize: 2048,
      timeout: Duration.seconds(30),
      environment: guardianEnvVars,
      initialPolicy: s3PolicyStatements,
      description: 'Codebase: https://github.com/guardian/manage-help-content-publisher.',
    });
    ...
    api.root.addResource('{articlePath}').addMethod('DELETE', new LambdaIntegration(takedownLambda), {
      apiKeyRequired: true,
    });
