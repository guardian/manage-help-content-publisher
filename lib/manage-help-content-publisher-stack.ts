...
export class ManageHelpContentPublisherStack extends GuStack {
  ...
  constructor(scope: App, id: string, props: ManageHelpContentPublisherStackProps) {
    ...
    const deploymentBucketName = (this.node.tryGetContext('deploymentBucket') as string) || 'membership-dist';
    ...
    const contentBucketName = (this.node.tryGetContext('contentBucket') as string) || 'manage-help-content';
    ...
    const s3PolicyStatements = [
      ...
      new PolicyStatement({
        ...
        resources: [
          deploymentBucket.arnForObjects('*'),
        ],
      }),
    ];
